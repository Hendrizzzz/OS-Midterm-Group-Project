...
# sort by MVP RATING (column 11) in descending order, and take the top 10 players
mvp_ladder=$(tail -n +2 "$input_file" | awk -F',' -v year="$year_to_filter" '$12 == year' | sort -t ',' -k 10 -nr | head -n 11)
