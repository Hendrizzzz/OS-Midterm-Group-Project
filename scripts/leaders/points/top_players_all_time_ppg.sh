...
awk -F, '
BEGIN {
    printf "%-25s %-30s %-10s %s\n", "Player Name", "Team", "Position", "Average PPG"
    printf "%-25s %-30s %-10s %s\n", "-----------", "----", "--------", "-----------"
}
...
END {
    # Store players and their average PPG in an array for sorting
    for (player in total_ppg) {
        num_seasons = length(seasons[player])  # Count distinct seasons
        average_ppg[player] = total_ppg[player] / num_seasons;
        split(team_position[player], info, "|");
        players[player] = average_ppg[player] "|" info[1] "|" info[2];  # Store avg PPG, team, and position
    }
    
    # Sort players based on average PPG and print the top 10
